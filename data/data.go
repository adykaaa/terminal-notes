...
func CreateTable() {
	createTableSQL := `CREATE TABLE IF NOT EXISTS tnote (
	...
	statement.Exec()
	log.Println("tnote table created")
}

func InsertNote(word string, definition string, category string) {
	insertNoteSQL := `INSERT INTO tnote(word, definition, category) VALUES (?, ?, ?)`
...
func DisplayAllNotes() {
	row, err := db.Query("SELECT * FROM tnote ORDER BY word")
	if err != nil {
		log.Fatal(err)
	}
